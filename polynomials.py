...
# Pledge   : I pledge my honor that I have abieded by the Stevens Honor System.
# Favorite flavor of dorito : Spicy Sweet Chili (The purple ones)
# *****
...
# exponent is strictly decreasing
    ...
    def __eq__(self, other): # myPoly == other
        ...
        for i in range(len(self.polyList)):
            if self.polyList[i].coef != other.polyList[i].coef or self.poLylist[i].exp != other.polyList[i].exp:
                return False
        return True;
    ...
    def __call__(self, input): # myPoly(input)
        ...
        x = 0
        for i in range(len(self.polyList)):
           x= x + Term.__call__(self.polyList[i], input)
        return x
    ...
    def __neg__(self): # negPoly = -myPoly
        ...
        newObj = LinkedPolynomial([])
        for i in range(len(self.polyList)):
            newObj.addTerm(Term(Term.__neg__(self.polyList[i]),self.polyList[i].exp))
        return newObj
    ...
    def __sub__(self, other): # subPoly = myPoly - other
        ...
        return LinkedPolynomial.__add__(self, LinkedPolynomial.__neg__(other))
